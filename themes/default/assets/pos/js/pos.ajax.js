$(document).ready(function(){
    ...
    $('#toogle-table-read-attr').click(function () {
        var nst = $('#postable').is('[readonly]') ? false : true;
        $('#postable').select2("readonly", nst);
        return false;
    });
    $(".open-brands").click(function () {
         if($(window).width() <= 768)
            {
                $('#brands-slider').css('width', $(window).width()-10);
            }else{
                $('#brands-slider').css('width', '540px');
            }
    ...
    $(".open-category").click(function () {
        if($(window).width() <= 768)
            {
                $('#category-slider').css('width', $(window).width()-10);
            }else{
                $('#category-slider').css('width', '540px');
            }
    ...
    $(".open-subcategory").click(function () {
        if($(window).width() <= 768)
            {
                $('#subcategory-slider').css('width', $(window).width()-10);
            }else{
                $('#subcategory-slider').css('width', '540px');
            }
        $('#subcategory-slider').toggle('slide', { direction: 'right' }, 700);
    });
    $(document).on('click', function(e){
        if (!$(e.target).is(".open-brands, .cat-child, .open-pic") && !$(e.target).parents("#brands-slider").size() && $('#brands-slider').is(':visible')) {
            $('#brands-slider').toggle('slide', { direction: 'right' }, 700);
        }
        if (!$(e.target).is(".open-category, .cat-child, .open-pic") && !$(e.target).parents("#category-slider").size() && $('#category-slider').is(':visible')) {
            $('#category-slider').toggle('slide', { direction: 'right' }, 700);
        }
        if (!$(e.target).is(".open-subcategory, .cat-child, .open-pic") && !$(e.target).parents("#subcategory-slider").size() && $('#subcategory-slider').is(':visible')) {
            $('#subcategory-slider').toggle('slide', { direction: 'right' }, 700);
        }
    });
...
if($('#poscustomer').val()){
 ...
 $.ajax({
            type: 'get',
            url: site.base_url+'pos/peruse_no',
            dataType: "json",
            data: { customer_id: $('#poscustomer').val()},
            success: function (data) {
               console.log(data);
            $('#help_block_today_peruse').html("Previous Peruse No. : "+data.peruse_no);
 ...
 * Load all items
 ----------------------- */
function  get_peruse_no(customer_id){
console.log(site.base_url+'pos/peruse_no');
           $.ajax({
            type: 'post',
            url: site.base_url+'pos/peruse_no',
            dataType: "html",
...
//console.log(localStorage.getItem('positems'));
        ...
        var customer_group_name = "General";
        $.each(sortedItems, function () {
            ...
            var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.price, item_price_before_discount = item.row.price_before_discount, customer_g_discount = item.row.customer_group_discount, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            var product_unit = item.row.unit, base_quantity = item.row.base_quantity;
            customer_group_name = item.row.customer_group_name;
            // Mahiruho - Gaurav
            ...
            var unit_price_before_discount = item.row.price_before_discount;
            ...
            if(item.row.fup != 1 && product_unit != item.row.base_unit) {
                $.each(item.units, function(){
                    if (this.id == product_unit) {
                        ...
                        unit_price_before_discount = formatDecimal((parseFloat(item.row.price_before_discount)*(unitToBaseQty(1, this))), 4);
                ...
                $.each(item.options, function () {
                    if(this.id == item.row.option && this.price != 0 && this.price != '' && this.price != null) {
                        ...
                        unit_price = item_price;
                         ...
                         item_price_before_discount = unit_price_before_discount+(parseFloat(this.price));
                        unit_price_before_discount = item_price_before_discount;
            ...
            unit_price_before_discount = formatDecimal(unit_price_before_discount-item_discount);
            ...
            item_price_before_discount = item_tax_method == 0 ? formatDecimal((item_price_before_discount-pr_tax_val), 4) : formatDecimal(item_price_before_discount);

            unit_price = formatDecimal((unit_price+item_discount), 4);
            unit_price_before_discount = formatDecimal((unit_price_before_discount+item_discount), 4);
            var sel_opt = '';
            $.each(item.options, function () {
            ...
            // Mahiruho - Gaurav
            var newTr = $('<tr id="row_' + row_no + '" class="row_' + item_id + '" data-item-id="' + item_id + '" data-image="'+image+'"></tr>');
            // Mahiruho - Gaurav
            ...
            customer_group_discount += formatDecimal(((parseFloat(customer_g_discount) + parseFloat(pr_tax_val)) * parseFloat(item_qty)), 4);
           ...
           // alert(pos_settings.remote_printing);
            if (pos_settings.remote_printing != 1) {
        ...
        customer_group_discount =  formatDecimal(customer_group_discount);
...
/*added by sudip end*/
         ...
         $('#customer_group_discount').text(formatMoney(customer_group_discount));
...
function showpanel22() {
         ...
         $.ajax({
            type: "GET",
            global: false,
            url: site.base_url+'auth/log'
            }).done( function( data ){
                //alert(data);
            });
     }
 // use setTimeout() to execute
 setInterval(showpanel22, 10000)