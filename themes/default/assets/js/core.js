...
//if (ww > 1024 && Cookies.get("sma_sidebar") != 'minified') {
    ...
    $(document).on('click', '.po-delete', function(e) {
        ...
        $.ajax({type: "get", url: link, dataType: 'json',
            success: function(data) {
                ...
                else { addAlert(data.msg, 'success'); 
                if(data.ff!='' ||data.ff!= undefined ){
                    setTimeout(function(){
                      document.location.href = data.ff;
                    }, 1000);
...
function table_status(x) {
    var y = x.split("__");
    return y[0] == 1 ?
    '<a href="'+site.base_url+'restaurant/deactivate/'+ y[1] +'" data-toggle="modal" data-target="#myModal"><span class="label label-success"><i class="fa fa-check"></i> '+lang['active']+'</span></a>' :
    '<a href="'+site.base_url+'restaurant/activate/'+ y[1] +'"><span class="label label-danger"><i class="fa fa-times"></i> '+lang['inactive']+'</span><a/>';
}
...
$(document).ready(function() {
    ...
    $('body').on('click', '.receipt_link td:not(:first-child, :last-child)', function() {
        ...
        jQuery('#myModal').modal({ remote: site.base_url + 'pos/view/' + $(this).parent('.receipt_link').attr('id') + '/1' });
    });
