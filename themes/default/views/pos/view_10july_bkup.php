    ...
    <div id="wrapper">
        <div id="buttons" style="padding-top:10px; text-transform:uppercase;" class="no-print">
            ...
            if ($modal) {
                ...
                <span class="pull-left col-xs-12"><a class="btn btn-block btn-success" href="javascript:;" id="send_sms"  onclick="sendSMS(this)">Send SMS</a></span>
...
//var_dump($customer);
                ...
                if(!empty($order_type_setting)){
                    echo 'Order Type : '.$order_type_setting.'<br>';
                }
                echo "</p>";
                echo "<p>";
    ...
    echo 'Customer Group Name : '.$customer->customer_group_name.'<br>';
                ...
                echo '</div><div style="width: 50%;float: left;">';
                    if(!empty($table) && isset($table)){
                        echo '<strong>Table Info : </strong><br>';
                        //print_r($table);
                        if($table->table_name){
                            echo 'Table Name : '.$table->table_name.'<br>';
                        }
                        if($table->no_of_guest){
                            echo 'No Of Guest : '.$table->no_of_guest.'<br>';
                        }
                        if($table->email){
                            echo 'Email : '.$table->email.'<br>';
                        }
                        if($table->phone){
                         echo 'Phone : '.$table->phone.'<br>';
                        }
                        if($table->phone){
                           // echo 'Phone : '.$table->phone.'<br>';
                        }
                        if($table->start_time && $table->end_time){
                            echo 'Time : '.$table->start_time.' - '.$table->end_time.' <br>';
                        }
                    }
                    echo '</div></div>';
                //}
