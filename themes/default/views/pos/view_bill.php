...
<head>
    ...
    <style type="text/css">
        ...
        h2 { margin: 20px 0; }
        .with-promo-space { }
        .bill { min-height: 540px; background: #FFF; margin: 20px 20px 0 20px; }
        ...
        #product-list { display: block; overflow: hidden; min-height: 426px; border: 1px solid #ddd; padding: 20px 20px 0 20px; }
        #totals { border-top: 1px solid #ddd;     font-size: 110%;}
...
<div class="wrap">
    ...
    <div class="main">
        <!-- If you don't want to display customer details, please remove id="rightside-content" the id from the div below -->
...
if(localStorage.getItem('posshipping') == null)
{
    shipping = 0;
}
        ...
        if (localStorage.getItem('positems')) {
            ...
            $.each(sortedItems, function () {
               ...
               // console.log(item);
                ...
                var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, customer_g_discount = item.row.customer_group_discount, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
                var product_unit = item.row.unit, base_quantity = item.row.base_quantity;
                var unit_price = item.row.real_unit_price;
                if(item.row.customer_group_name){
            ...
            //var gtotal = parseFloat(((total + invoice_tax) - order_discount)) + (shipping==NaN?0:shipping);


            if(isNaN(shipping) == true){
            shipping = 0;
            }
//alert('(('+total+' +'+ invoice_tax+') -'+ order_discount+') +'+ isNaN(shipping));
        ...
        $.ajax({url: site.base_url+"/reports/restCurrenciesAll", success: function (result) {
                ...
                if (result) {
                    ...
                    if (len > 0) {
                        for (var i = 0; i < len; i++) {
                            ...
                            oCurT += '<tr class="curr_'+oCur[i].code+' removeThis"><td colspan="3" style="border-left: 1px solid #ddd; padding-left:10px; text-align:left; font-weight:normal;">'+oCur[i].code+'</td><td align="right" style="text-align:right; padding-right:10px; font-weight:bold;">'+parseFloat(oCur[i].rate*gTotal).toFixed(2)+'</td></tr>';
                        }
                        $('tr.removeThis').remove();
                        $('tr#discount_row').after(oCurT);
...
<script>
    ...
    setInterval(function() { shipping = parseFloat(localStorage.getItem('posshipping')); }, 2000);
</script>
</body>
</html>
