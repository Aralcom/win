...
//var_dump($suspend_sale);
$pos_order_type_db = json_decode($pos_settings->order_type_setting);
//var_dump($pos_order_type_db);
//$pos_settings->restaurant_model
//echo $pos_settings->restaurant_model;
?>
...
<?php }
    ...
    <script src="<?php echo base_url('themes/default/assets/js/printhis.js')?>"></script>
    <link type="text/css" rel="stylesheet" href="<?=$assets?>pos/css/layout-default.css" />     
...
.Closediv{ display: none; all: unset;
 }
 @media print {
        .no-print { display: none; }
        #pos-sale-form{display:none;}
        #wrapper,#receiptData,#container {float:none!important;max-width: 480px!important; width: 100%!important; min-width: 250px!important; margin: 0 auto!important; }
        .no-border { border: none !important; }
        .border-bottom { border-bottom: 1px solid #ddd !important; }
    }
.ui-layout-resizer{ display: none; }
.calculator-result{float: left; height:  25px; }
.calculator-formula{ width: 100%; position: relative; float: left;}
.calculator-formula span{ display: none; }
.calculator-digit{height: 20px; }
@media (max-width: 769px)  {
  .btn.pos-tip.btnicon3 { width: 100%; }
  .panel.panel-default{ margin-left: -30%; width: 140%; }
...
<!--<script type="text/javascript" src="http://layout.jquery-dev.net/lib/js/jquery-ui-latest.js"></script>-->
      ...
      if(isset($pos_resize->left_shorting) && !empty($pos_resize->left_shorting)){
          $json_left_shorting = json_decode($pos_resize->left_shorting);
          
      }else{
        $json_left_shorting = json_decode('{"1":"short_left_1","2":"short_left_2","3":"short_left_3"}');

      }
      if(isset($pos_resize->right_shorting) && !empty($pos_resize->right_shorting)){
          $json_right_shorting = json_decode($pos_resize->right_shorting);
          
      }else{
        $json_right_shorting = json_decode('{"1":"short_left_1","2":"short_left_2"}');
        
      }
      if(isset($pos_resize->box_size) && !empty($pos_resize->box_size)){
          $json_box_size = json_decode($pos_resize->box_size);
          
      }else{
        $json_box_size = json_decode('{"left_width":"300px","right_width":"300px"}');
        
      }
  	  if(isset($pos_resize->box_open_opt) && !empty($pos_resize->box_open_opt)){
          $box_open_opt = json_decode($pos_resize->box_open_opt);
          
      }else{
        $box_open_opt = json_decode('{"calulatorBox":"panel-collapse collapse","customerInfo":"panel-collapse collapse","productInfo":"panel-collapse collapse","tableInfo":"panel-collapse collapse","moreOptions":"panel-collapse collapse","settingsButtons":"panel-collapse collapse"}');
      ...
      <script type="text/javascript">
 $(document).ready(function(){
                $.layout.defaults.west.size = '<?php echo str_replace("px","",$json_box_size->left_width);?>';
                $.layout.defaults.east.size = '<?php echo str_replace("px","",$json_box_size->right_width);?>';
                
                //resize_able
                $('#container').layout();
              });
               function save_position(){
                  var current_left_width = $('#leftdiv2').css('width');
                  var current_right_width = $('#rightBox').css('width');
                  var myObj2 = {};
                  myObj2['left_width'] = current_left_width;
                  myObj2['right_width'] = current_right_width;
                  var json_width = JSON.stringify(myObj2);
                  var myObj = {};
                  var short_id = 1;
                 var total_li = $("#sortable li").length;
                 
                  $('#sortable li').each( function(e) {
                    myObj[short_id] = $(this).attr('id');
                    short_id++;
                  ...
                  var json_shortable = JSON.stringify(myObj);
                 // alert(json_shortable);
                  var myObj1 = {};
                  var short_id1 = 1;
                  $('#sortable1 li').each( function(e) {
                    myObj1[short_id1] = $(this).attr('id');
                    short_id1++;
                  });
                  var json_shortable1 = JSON.stringify(myObj1);
				  //box+open+close
					var boxObj = {};
					boxObj['calulatorBox'] = $('#calulatorBox').attr('class');
					boxObj['customerInfo'] = $('#customerInfo').attr('class');
					boxObj['productInfo'] = $('#productInfo').attr('class');
					boxObj['tableInfo'] = $('#tableInfo').attr('class');
					boxObj['moreOptions'] = $('#moreOptions').attr('class');
					boxObj['settingsButtons'] = $('#settingsButtons').attr('class');
					var json_boxObj = JSON.stringify(boxObj);
				  //box+open+close
                  $('#modal-loading').show();
                  $.ajax({
                    type: "get",
                    url: "<?=site_url('restaurant/update_pos_resize')?>",
                    data: {json_width: json_width, json_shortable1: json_shortable1, json_shortable: json_shortable ,json_boxObj: json_boxObj},
                    dataType: "html",
...
<body>
  ...
  <!-- <header id="header" class="navbar">
        ...
        <?php
          ...
          <input type="hidden" name="quick_pay_ajax" value="3">
		  <input type="hidden" name="suspend_sale_id" value="<?php if(isset($sid)){echo $sid;}?>">
         
            <?php /*<div class="col-sm-4" id="leftdiv2" class="pos-box">*/ ?>
            <div class="pane ui-layout-west" id="leftdiv2" style="min-width: 31.6%; max-width: 31.6%; height: 100%;">
            <!-- <div class="row"> -->
            ...
            <div class="col-sm-12" id="calculator">
              <h1><a href="<?=site_url()?>" class="pos-header">
                ...
                <input type="button" name="" onclick="save_position();" value="Save">
              <div class="panel-group">
                  <ul id="sortable" class="ui-sortable">
                   <div class="panel panel-default ui-state-default ui-sortable-handle">
                      <div class="panel-heading"> <a href="#customerInfo" data-toggle="collapse" data-parent="#detailGroup">
                        <h4 class="panel-title">
                          <?=lang('customer_detail')?>
                        </h4>
                        </a> </div>
                      <div id="customerInfo" class="<?php echo ($box_open_opt->customerInfo?$box_open_opt->customerInfo:' panel-collapse collapse')?>">
                ...
                <div class="panel panel-default">
                  ...
                  <div id="calulatorBox" class="<?php echo ($box_open_opt->calulatorBox?$box_open_opt->calulatorBox:' panel-collapse collapse')?>">
            ...
            <div class="col-sm-12" id="customerDetail">
              <div class="panel-group" id="detailGroup">
                 <ul id="sortable" class="ui-sortable">
                  <?php foreach($json_left_shorting as $k1=>$val1){
            if($val1 == 'short_left_1'){
              ?>
               ...
               <?php
             } else if($val1 == 'short_left_2'){
              ?>

              <?php
             if(isset($pos_settings->restaurant_model) && $pos_settings->restaurant_model == 1){
               ?>
                <li class="ui-state-default ui-sortable-handle" id="short_left_2">
                    ...
                    <div class="panel panel-default ui-state-default ui-sortable-handle" >
                      ...
                      <div id="tableInfo" class="<?php echo ($box_open_opt->tableInfo?$box_open_opt->tableInfo:' panel-collapse collapse')?>">
                        <div class="panel-body">
                          <div class="media">
                            <div class="media-body">
                            <!--
                            -->
                              ...
                              <!--
                            -->
                                  <h4 class="media-heading" id="gkTableName"><strong><?php echo ($suspend_sale)?$suspend_sale->table_name:'';?></strong></h4>
                                  <h4>
                                    
                                  </h4>
                                  <p class="" id="gkno_of_guest"><strong><?php echo ($suspend_sale)?$suspend_sale->no_of_guest:'';?></strong></p>
                                  <h4>
                                    
                                  </h4>
                                  <p class="" id="gkTableAddress"><strong><?php echo ($suspend_sale)?$suspend_sale->address:'';?></strong></p>
                                  <h4>
                                    
                                  </h4>
                                  <p class="" id="gkTablePhone"><strong><?php echo ($suspend_sale)?$suspend_sale->phone:'';?></strong></p>
                                  <h4>
                                    
                                  </h4>
                                  <p class="" id="gkTableMail"><strong><?php echo ($suspend_sale)?$suspend_sale->email:'';?></strong></p>
                              </div>
                            </div>
                            <div class="media-right" style="display:none;"> <img id="tableImage" src="<?=site_url('assets/uploads/no_image.png')?>" class="media-object" style="width: 100px;"> </div>
                          </div>
                            <div class="form-group">
                              <?php
                              echo form_input('table', (isset($_POST['table']) ? $_POST['table'] : ""), 'id="postable" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("table") . '" required="required" class="form-control pos-input-tip" style="width:100%;" value="'.$table->id.'"');
                                                                  ?>
                            </div>
                          <div class="row">
                            <div class="col-xs-6">
                              <button type="button" id="toogle-table-read-attr" class="btn btnicon fullWidth" title="Change Table"> <i class="fa fa-pencil"></i> </button>
                            </div>
                            <?php //if ($Owner || $Admin || $GP['customers-add']) { ?>
                            <div class="col-xs-6"> <a href="<?=site_url('restaurant/add');?>" id="add-customer" class="btn btnicon fullWidth" data-toggle="modal" data-target="#myModal" title="Add Table"> <i class="fa fa-user-plus"></i> </a> </div>
                            <?php //} ?>
                          </div>
                        </div>
                      </div>
               ...
               ?>   
                  <li class="ui-state-default" id="short_left_3">  
                    ...
                    <div class="panel panel-default">
                      ...
                      <div id="productInfo" class="<?php echo ($box_open_opt->productInfo?$box_open_opt->productInfo:' panel-collapse collapse')?>">
                        <div class="panel-body">
                          <div class="media">
          ...
          <div class="pane ui-layout-center" id="leftdiv" style="min-width: 34%; max-width: 34%;">
          ...
          <div id="left-top">
            ...
            <div class="no-print">
              ...
              <?php } else {
                       ...
                       if(!empty($pos_settings->order_type_setting) && isset($pos_settings->restaurant_model) && $pos_settings->restaurant_model == 1){
                       $order_type_setting = json_decode($pos_settings->order_type_setting);
                       
                       ?>      
              <div class="form-group">
                <?php
                                 $order_type_value = (array)$order_type_setting[0]; 
                                 $order_type_value1 = array('0' => 'Select Order Type');
                                 $order_type_value = array_merge($order_type_value1,$order_type_value);
                                 
                                
                                    echo form_dropdown('order_type_setting', $order_type_value , (isset($_POST['order_type_setting']) ? $_POST['order_type_setting'] : $order_type_setting[1]->selected), 'id="order_type_setting" class="form-control pos-input-tip" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("order_type") . '"  style="width:100%;" ');
                                ?>
              </div>  
                  <?php
                  }
     ...
     <?php /*<div class="col-sm-3" id="" class="pos-box">*/ ?>
        ...
        <div class="pane ui-layout-east" id="rightBox" style="min-width: 32.70%; max-width: 32.70%; ">  
        
        <div class="panel-group">
        <!-- shortable -->
          <ul id="sortable1">
          <?php foreach($json_right_shorting as $k=>$val){
            if($val == 'short_left_1'){
              ?>
                <li class="ui-state-default" id="short_left_1">
          <div class="panel panel-default">
            ...
            <div id="moreOptions" class="<?php echo ($box_open_opt->moreOptions?$box_open_opt->moreOptions:' panel-collapse collapse')?>">
              <div class="panel-body">
                ...
                <input type="hidden" name="username" id="username" value="<?= ($user->username) ? $user->username : $user->email;?>"/>
                ...
                <button class="btn pos-tip btnicon3" style="width: 49%" type="button" id="sellGiftCard" title="<?=lang('sell_gift_card')?>"> <img src="<?=$assets?>images/Gift_card.png" >
                <?//=lang('sell_gift_card')?>
                </button>
                <button class="btn pos-tip btnicon3" style="width: 49%" type="button" id="quick_pay_new"><i class="fa fa-money" style="margin-right: 5px;"></i>Quick Pay</button>
                <?php } ?>
              ...
              <?php
            }else if($val == 'short_left_2'){
              ?>
              <li class="ui-state-default" id="short_left_2">
              <div class="panel panel-default">
              ...
              <div id="settingsButtons" class="<?php echo ($box_open_opt->settingsButtons?$box_open_opt->settingsButtons:' panel-collapse collapse')?>">
                <div class="panel-body"> 
                  <!-- #1 --> 
                  ...
                  <div class="dropdown-menu calc">
                    <div class="dropdown-content"> <span id="inlineCalc" style="width: 100%; margin: 0px -5px; position: absolute; clear: both;"></span> </div>
                  </div>
                </div>
              </div>
            </div>
...
<div class="modal fade in" id="quickpaymentModal" tabindex="-1" role="dialog" aria-labelledby="quickpaymentModal" aria-hidden="true">
...
<div class="modal fade in" id="sckModal" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content" id="shortcut_print">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true"> <i class="fa fa-2x">&times;</i></span><span class="sr-only">
        <?=lang('close');?>
        </span> </button>
        <button type="button" class="btn btn-xs btn-default no-print pull-right" style="margin-right:15px;" onclick="$('#shortcut_print').printThis();"> <i class="fa fa-print"></i>
        <?= lang('print'); ?>
        </button>
        <h4 class="modal-title" id="mModalLabel">
          <?=lang('shortcut_keys')?>
...
<div id="order_tbl"><span id="order_span"></span> <span style="text-align:center">
  <h4>Invoice No: <?php echo ($totalSaleCount->id + 1);?> <br/>
    Reference No: S/POS/00<?php echo ($totalSaleCount->id + 1);?>
    <br><div class="table_name_order"> </div><br>
  </span>
...
<div id="bill_tbl"><span id="bill_span"></span> <span style="text-align:center">
  <h4>Invoice No: <?php echo ($totalSaleCount->id + 1);?> <br/>
    Reference No: S/POS/00<?php echo ($totalSaleCount->id + 1);?>
    <br><div class="table_name_order"> </div><br></h4>
  </span>
...
<script type="text/javascript">
    ...
    var lang = {unexpected_value: '<?=lang('unexpected_value');?>', select_above: '<?=lang('select_above');?>', r_u_sure: '<?=lang('r_u_sure');?>', bill: '<?=lang('bill');?>', order: '<?=lang('order');?>', total: '<?=lang('total');?>',discount: '<?=lang('discount');?>',grand_total: '<?=lang('grand_total');?>',items: '<?=lang('items');?>'};
...
<script type="text/javascript">
        ...
        function widthFunctions(e) {
            ...
            $('#left-middle').css("height", wh - lth - lbh - 125);
            $('#left-middle').css("min-height", 170);
            $('#product-list').css("height", wh - lth - lbh - 132);
            $('#product-list').css("min-height", 170);
            ...
            $('#leftdiv2, #rightBox').css('height','auto');
        }
    ...
    <?php if ($this->session->userdata('remove_posls')) {?>
            ...
            localStorage.setItem('postable', '<?=$suspend_sale->table_id;?>');
           ...
           <?php if ($this->input->get('customer')) {?>
			...
			if (!localStorage.getItem('username')) {
                localStorage.setItem('username', '<?= ($user->username) ? $user->username : $user->email;?>');
            }
			<?php	
			}
            ?>
          ...
          // alert(localStorage.getItem('postable'));
           ...
           console.log(localStorage.getItem('postable'));
                ...
                if($('input[name=customer]').val() != '')
                    {
                        $.ajax({
                            ...
                            success: function(res){
                                ...
                                //$('#gkId strong').html(res.table.id);
                                $('#gkTableAddress strong').html('Address : '+res.table.address);
                                $('#tableImage').attr('src','<?=site_url("assets/uploads/avatars/")?>' + res.table.avatar);
                            }
                        });
                    }
            });
            ...
            $('#payment').click(function () {
                ...
                $.ajax({url: "reports/restCurrenciesAll", success: function (result) {
                ...
                if (total_paid == 0 || total_paid < grand_total) {
                    bootbox.confirm("<?=lang('paid_l_t_payable');?>", function (res) {
                        if (res == true) {
                            ...
                            //console.log($('#pos-sale-form').serialize());
            ...
            $(document).on('click', '#quick_pay_new', function () {
              $('#modal-loading').show();
              $('#pos_note').val(localStorage.getItem('posnote'));
              $('#staff_note').val(localStorage.getItem('staffnote'));
              var gTotala=$('#gtotal').text();
              ...
              $('#balance_amount_1').val('0');
              console.log(gTotal);
              $('#quick_pay_ajax').val('2');
              var data = $('#pos-sale-form').serialize();
              $.ajax({
                url: $('#pos-sale-form').attr('action'),
                data: data,
                type: 'POST',
                dataType: 'json',
                success: function(response){
                  ...
                  var new_sale_id = response.id;
                  console.log(new_sale_id);
                  $.ajax({
                    url: BASE_URL + 'pos/view_ajax/'+new_sale_id+'?modal='+new_sale_id,
                    type: 'GET',
                    success: function(viewRes){
						console.log('viewRes',viewRes);
						$('#quickpaymentModal .modal-body').empty();
						$('#quickpaymentModal .modal-body').append(viewRes);
						...
						$('#amount_val_1').val('');
						$('#balance_amount_1').val('0');
						$('#quick_pay_ajax').val('3');
						loadItems();
						setTimeout(function(){$('#quickpaymentModal').modal('hide');},1000);
       ...
       <?php if ($pos_settings->java_applet) {?>
        ...
        $(document).ready(function () {
            $('#print_order').click(function () {
                ...
                <?php
                if($val1 == 'short_left_3'){
                ?>
                $('.table_name_order').html("Table "+$('#gkTableName').text());
                $('.table_name_order').css('display','block');
                <?php 
                }
                ?>
        ...
        function select_table(id){
         $.ajax({
                            url : site.base_url + 'restaurant/tableDetail/' + id,
                            type : 'get',
                            dataType : 'json',
                            success: function(res){
                                console.log(res);
...
<script type="text/javascript">
        ...
        $(document).ready(function(){
           $(".ui-layout-resizer").addClass( "Closediv");
           $(".ui-layout-toggler").addClass("Closediv");
           $(".ui-layout-resizer-west").addClass("Closediv");
           $(".ui-layout-resizer-open").addClass("Closediv");
           $(".ui-layout-resizer-west-open").addClass("Closediv");
           $(".ui-layout-east").css({"margin-right":"10px"});
           $(".panel-group #sortable").addClass("ui-sortable");
           //$(".btn.pos-tip.btnicon3").css({"width":"100%"});
        });
    </script> 
