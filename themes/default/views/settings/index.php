                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="alert alert-info" role="alert"><p>
                            ...
                            <strong>Cron Job:</strong> <code>0 0 * * * wget -qO- <?= site_url('cron/backup'); ?>  &gt;/dev/null 2&gt;&amp;1</code> to run cronjob backup daily. For local installation, you can run cron job manually at any time.
                            <?php if (!DEMO) { ?>
                                <a class="btn btn-primary btn-xs pull-right" target="_blank" href="<?= site_url('cron/backup'); ?>">Run cron job backup now</a>
                            <?php } ?>
                            <div class="clearfix"></div>
                            <strong>Cron Job:</strong> <code>0 0 * * 0 wget -qO- <?= site_url('cron/backup'); ?>  &gt;/dev/null 2&gt;&amp;1</code> to run cronjob backup weekly. For local installation, you can run cron job manually at any time.
                            <?php if (!DEMO) { ?>
                                <a class="btn btn-primary btn-xs pull-right" target="_blank" href="<?= site_url('cron/backup'); ?>">Run cron job backup now</a>
                            <?php } ?>
                        </p></div>
...
<?php /*added by sudip start*/?>
<div class="col-md-4">
                        <div class="form-group">
                            <?= lang("Logo", "Logo"); 

?>
<img src="<?= base_url('assets/uploads/logos/' . $Settings->logo); ?>" alt="<?= $Settings->company != '-' ? $Settings->company : $Settings->name; ?>">
...
?>
<textarea name="address" id="address" data-show-upload="false" data-show-preview="false" class="form-control file"  data-browse-label="<?= lang('browse'); ?>" ><?php echo $Settings->address;?></textarea>
...
<div class="col-md-4">
<div class="form-group">
                            <?= lang("Total Balance Text", "Total Balance Text"); 

?>
<input type="text" name="text_total_bal" id="text_total_bal" data-show-upload="false" data-show-preview="false" class="form-control file"  data-browse-label="<?= lang('browse'); ?>" value="<?php echo $Settings->text_total_bal;?>" >
...
<div class="col-md-4">
<div class="form-group">
                            <?= lang("Grand Total Text", "Grand Total Text"); 

?>
<input type="text" name="text_grand_total" id="text_grand_total" data-show-upload="false" data-show-preview="false" class="form-control file"  data-browse-label="<?= lang('browse'); ?>" value="<?php echo $Settings->text_grand_total;?>" >
...
<div class="col-md-4">
<div class="form-group">
                            <?= lang("Customer Group Option", "Customer Group Option"); 

?>
<select name="customer_group_opt" data-show-upload="false" data-show-preview="false" class="form-control file"  data-browse-label="<?= lang('browse'); ?>" >
<option value="1" <?php if($Settings->customer_group_opt==1){?> selected="selected" <?php }?>>Show</option>
<option value="0" <?php if($Settings->customer_group_opt==0){?> selected="selected" <?php }?>>Hide</option>
</select>
...
<div class="col-md-4">
                        <div class="form-group">
                            ...
                            <?php
                            $report_delete_button[""] = "";
                            foreach (array('Disable','Enable') as $key=>$biller) {
                                $report_delete_button[$key] = $biller;
                            }
                            echo form_dropdown('report_delete_button', $report_delete_button, (isset($_POST['report_delete_button']) ? $_POST['report_delete_button'] : $Settings->report_delete_button), 'id="apis" data-placeholder="' . lang("select") . '" required="required" class="form-control input-tip select" style="width:100%;"');
                            ?>
                        </div>
                    </div>
<div class="col-md-4">
                        <div class="form-group">
                            ...
                            <?php
                            $apis[""] = "";
                            foreach (array('daily'=>'Daily','wekkly'=>'Weekly') as $key=>$biller) {
                                $apis[$key] = $biller;
                            }
                            echo form_dropdown('backup_time', $apis, (isset($_POST['backup_time']) ? $_POST['apis'] : $Settings->backup_time), 'id="backup_time" data-placeholder="' . lang("select") . ' Backup Schedule" required="required" class="form-control input-tip select" style="width:100%;"');
                            ?>
                        </div>
                    </div>
<div class="col-md-4">
                        <div class="form-group">
                            ...
                            <?= form_input('backup_email', $Settings->backup_email, 'class="form-control tip" id="backup_email"  required="required"'); ?>
                        </div>
                    </div>
<div class="col-md-4">
                        <div class="form-group">
                            <label>API Featured</label>
                            <?php
                            $apis[""] = "";
                            foreach (array('Disable','Enable') as $key=>$biller) {
                                $apis[$key] = $biller;
                            }
                            echo form_dropdown('apis', $apis, (isset($_POST['apis']) ? $_POST['apis'] : $Settings->apis), 'id="apis" data-placeholder="' . lang("select") . ' API Featured" required="required" class="form-control input-tip select" style="width:100%;"');
                            ?>
                        </div>
                    </div>
<div class="col-md-4">
                        <div class="form-group">
                            <label>SHOP Featured</label>
                            <?php
                            $shop_feature[""] = "";
                            foreach (array('Disable','Enable') as $key=>$biller) {
                                $shop_feature[$key] = $biller;
                            }
                            echo form_dropdown('shop_feature', $shop_feature, (isset($_POST['shop_feature']) ? $_POST['shop_feature'] : $Settings->shop_feature), 'id="shop_feature" data-placeholder="' . lang("select") . ' Shop Featured" required="required" class="form-control input-tip select" style="width:100%;"');
                            ?>
                        </div>
                    </div>
<div class="col-md-4">
<div class="form-group">
                            <?= lang("Payment Due Notification(Before *Days)", "due_payment_notification_alert"); 

?>
<div style="display: inline-flex;">Notify Before &nbsp;<input style="width:60px;" type="text" name="due_payment_notification" id="due_payment_notification" data-show-upload="false" data-show-preview="false" class="form-control file"  data-browse-label="<?= lang('browse'); ?>" value="<?php echo $Settings->due_payment_notification;?>" > &nbsp;Day's</div>
                           
                        </div>
                    </div>

<?php /*added by sudip end*/?>
                    </fieldset>
