...
foreach($rows as $row){
    ...
    $query = $this->db->query("SELECT box_status,box_quantity FROM `sma_products` WHERE id='$product_id'");
    $product_detail = $query->row_array();
    $dt_quantity = $this->sma->formatQuantity($row->quantity);
    if($product_detail){
        if($product_detail['box_status'] == 1 && $dt_quantity >= $product_detail['box_quantity']){
            ...
            if(strpos($box_number,'.') !== false){
                $box_number = substr($box_number,0,1);
                    ...
                    <table class="table table-striped table-condensed">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo $box_product['box_number'];?></td>
                                <td><?php echo $box_product['extra_number'];?></td>
                            </tr>
                            <?php endforeach?>
                        </tbody>
                    </table>
