...
<?php
 $aa = $this->db->query("SELECT * FROM `sma_gift_cards` WHERE expiry < NOW()");
 $result = $aa->result();
 ...
 if($result)
 {
    ...
    {
        $initial_balance = $initial_balance + $row->balance;
    }
 }
?>
                    ...
                    <div class="col-sm-12">
                        ...
                        <div class="col-sm-3">
                            <div class="small-box padding1010 bpurple">
                                ...
                                <h3 class="bold"><?= $this->sma->formatMoney($initial_balance) ?></h3>
                                <p>(<?= $this->sma->formatMoney($initial_balance) . ' ' . lang('gift_card') ?>
                                    - <?= $this->sma->formatMoney($total_sales->tax) . ' ' . lang('tax') ?>)</p>
                            </div>
                        </div>
                    </div>
...
<div class="row">
                    <div class="col-sm-12">
                        <div class="col-sm-12">
                            <div class="small-box padding1010 bpink">
                                <h4 class="bold">CAPITAL</h4>
                                <i class="fa fa-money"></i>
<?php
foreach ($warehouses_report as $warehouse_report) {
$total_amount_Expenses += $this->sma->formatMoney($warehouse_report['total_expenses']->total_amount);
}

?>
                                <h3 class="bold"><?= $this->sma->formatMoney($total_capital[0]->totalCapital-$total_capital[0]->totalWithdraw+$total_sales->total_amount - $total_purchases->total_amount - $total_expenses->total_amount + $initial_balance ) ?></h3>

                                <p><?= $this->sma->formatMoney($total_capital[0]->totalCapital)?> Capital - <?= $this->sma->formatMoney($total_capital[0]->totalWithdraw)?> Withdraw +<?= $this->sma->formatMoney($total_sales->total_amount) . ' ' . lang('sales') ?>
                                    - <?= $this->sma->formatMoney($total_purchases->total_amount) . ' ' . lang('purchases') ?> - <?= $this->sma->formatMoney($total_expenses->total_amount) . ' ' . lang('expenses').' + '.$this->sma->formatMoney($initial_balance) .' '. lang('gift_card')?><?php  //echo $this->sma->formatMoney($total_amount_Expenses).'Expenses';?> </p>
                            </div>
                        </div>
				...
				<div class="row">
					<div class="col-sm-12">
						<div class="col-sm-12">
							<div class="small-box padding1010 bblack">
								<h4 class="bold">Cash Flow</h4>
								<i class="fa fa-money"></i>
								<h3 class="bold">
								<?php
								echo $this->sma->formatMoney($total_capital[0]->totalCapital+$total_received_cash->total_amount-($total_capital[0]->totalWithdraw-$total_returned->total_amount+$total_paid->total_amount+$total_expenses->total_amount));

								?>
								</h3>
								<p>
								<?= $this->sma->formatMoney($total_capital[0]->totalCapital)?> Capital | <?=$this->sma->formatMoney($total_received_cash->total_amount)?> Sales by cash | <?=$this->sma->formatMoney($total_capital[0]->totalWithdraw)?> Withdraw | <?=$this->sma->formatMoney($total_returned->total_amount)?> Returned | <?=$this->sma->formatMoney($total_paid->total_amount)?> Sent | <?=$this->sma->formatMoney($total_expenses->total_amount)?> Expenses
								</p>
							</div>
						</div>
					</div>
