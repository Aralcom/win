...
class Reports_model extends CI_Model
{
    ...
    public function getTotalCapital($start, $end)
    {
    $qry="SELECT SUM( value ) AS totalCapital, (SELECT SUM( value ) FROM  `sma_capital` WHERE TYPE =  'out' and date between $start and $end ) AS totalWithdraw FROM  `sma_capital`  WHERE TYPE =  'in' and date between $start and $end";
//    return $qry;
    $dta=$this->db->query($qry);
    ...
    public function getSalesTotals($customer_id)
    {
        ...
        $this->db->select('SUM(COALESCE(grand_total, 0)) as total_amount, SUM(COALESCE(paid, 0)) as paid', FALSE)
            ->where('customer_id', $customer_id)->where('deleted',0);
        $q = $this->db->get('sales');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    {
        $this->db->from('sales')->where('customer_id', $customer_id)->where('deleted',0);
    ...
    {
        $this->db->from('sales')->where('customer_id', $customer_id)->where('sale_status', 'returned')->where('deleted',0);
        return $this->db->count_all_results();
    }
    ...
    public function getChartData()
    {
        ...
        FROM (  SELECT  date_format(date, '%Y-%m') Month,
                ...
                WHERE date >= date_sub( now( ) , INTERVAL 12 MONTH ) AND deleted=0
    ...
    public function getYearlySales($year, $warehouse_id = NULL)
    {
        if(!empty($year)){
            $year_array = json_decode($year);
        }
        $year = implode(',', $year_array);
        $myQuery = "SELECT DATE_FORMAT( date,  '%Y' ) AS date, SUM( COALESCE( product_tax, 0 ) ) AS tax1, SUM( COALESCE( order_tax, 0 ) ) AS tax2, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( total_discount, 0 ) ) AS discount, SUM( COALESCE( shipping, 0 ) ) AS shipping
            FROM " . $this->db->dbprefix('sales') . " WHERE ";
        $myQuery .= " deleted=0 AND ";
        if ($warehouse_id) {
            $myQuery .= " warehouse_id = {$warehouse_id} AND ";
        }
        $myQuery .= " DATE_FORMAT( date,  '%Y' ) IN  ({$year})
            GROUP BY date_format( date, '%Y' ) ORDER BY date_format( date, '%Y' ) ASC";
            //echo $myQuery;
        $q = $this->db->query($myQuery, false);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    public function getStaffMonthlySales($user_id, $year, $warehouse_id = NULL)
    {
     ...
     public function getStaffYearlySales($user_id, $year, $warehouse_id = NULL)
    {
        if(!empty($year)){
            $year_array = json_decode($year);
        }
        ...
        $myQuery .= " created_by = {$user_id} AND DATE_FORMAT( date,  '%Y' ) IN  ({$year})
            GROUP BY date_format( date, '%Y' ) ORDER BY date_format( date, '%Y' ) ASC";
          //echo $myQuery;  
        $q = $this->db->query($myQuery, false);
        if ($q->num_rows() > 0) {
    ...
    public function getCostingYear($date, $warehouse_id = NULL, $year = NULL, $month = NULL)
    {
        $this->db->select('SUM( COALESCE( purchase_unit_cost, 0 ) * quantity ) AS cost, SUM( COALESCE( sale_unit_price, 0 ) * quantity ) AS sales, SUM( COALESCE( purchase_net_unit_cost, 0 ) * quantity ) AS net_cost, SUM( COALESCE( sale_net_unit_price, 0 ) * quantity ) AS net_sales', FALSE);
        if ($date) {
            $this->db->where('costing.date', $date);
            ...
            $this->load->helper('date');
            $last_day = days_in_month(12, $year);
            $this->db->where("date_format( ".$this->db->dbprefix('costing').".date, '%Y' )=", $year );
            //$this->db->where('costing.date >=', $year.'-1-01 00:00:00');
            //$this->db->where('costing.date <=', $year.'-12-'.$last_day.' 23:59:59');
        }
    ...
    public function getExpensesYear($date, $warehouse_id = NULL, $year = NULL, $month = NULL)
    {
        $sdate = $date.' 00:00:00';
        $edate = $date.' 23:59:59';
        $this->db->select('SUM( COALESCE( amount, 0 ) ) AS total', FALSE);
        if ($date) {
           ...
           // $this->db->where('date >=', $year.'-1-01 00:00:00');
            //$this->db->where('date <=', $year.'-12-'.$last_day.' 23:59:59');
        }
    ...
    public function getReturnsYear($date, $warehouse_id = NULL, $year = NULL, $month = NULL)
    {
        $sdate = $date.' 00:00:00';
        $edate = $date.' 23:59:59';
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total', FALSE)
    ...
    public function getOrderDiscountYear($date, $warehouse_id = NULL, $year = NULL, $month = NULL)
    {
        $sdate = $date.' 00:00:00';
        $edate = $date.' 23:59:59';
        $this->db->select('SUM( COALESCE( order_discount, 0 ) ) AS order_discount', FALSE);
