...
class Sales_model extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id, $limit = 5)
    {
        ...
        $this->db->select('products.*, FWP.quantity as quantity, categories.id as category_id, categories.name as category_name', FALSE)
            ...
            ->group_by('products.id, FWP.quantity');
...
//                        }
            ...
            if (!empty($si_return)) {
                ...
                // Insert sale return
                $this->db->insert('return_sale',array('return_sale_ref' => $data['return_sale_ref'], 'surcharge' => $data['surcharge'],'return_sale_total' => $data['grand_total'], 'return_id' => $sale_id,'sale_id'=>$data['sale_id']));
                    ...
                    foreach ($item_costs as $item_cost) {
                        if (isset($item_cost['date'])) {
                            ...
                            foreach ($item_cost as $ic) {
                                $ic['sale_item_id'] = $sale_item_id;
                                $ic['sale_id'] = $id;
                                if(count($ic) > 0 && !isset($ic['pi_overselling'])) {
                                    $this->db->insert('costing', $ic);
                                }
                            }
                        }
    ...
    public function deleteSale($id)
    {
        ...
        elseif($this->db->update('sales', array('deleted' => 1), array('id' => $id))) {
            return true;
        }
        else {
            return false;
	...
	public function deleteSaleDe($id)
    {
        //$sale_items = $this->resetSaleActions($id);
        $inv = $this->getInvoiceByID($id);
        if($inv->deleted == 1){
