...

{
    ...
    public function run_backup(){
        $this->load->dbutil();
        if(isset($this->Settings->backup_email) && $this->Settings->backup_email != ''){
            $prefs = array(
            ...
            $save_database = FCPATH.'/files/backups/' . $db_name;

            $this->load->helper('file');

            write_file($save_database, $backup);



            $files = glob(FCPATH.'/files/backups/*.txt', GLOB_BRACE);
            ...
            $config['useragent'] = "Business Management System";
            ...
            $parse_data = array(
                ...
                'email' => $this->Settings->backup_email,
            ...
            $subject = 'Backup database cronjob - '.$this->Settings->site_name;



            $this->email->from($this->Settings->default_email, $this->Settings->site_name);

            $this->email->to($this->Settings->backup_email);

            $this->email->subject($subject);

            $this->email->message($message);
            $this->email->attach($save_database);
...

    {
        ...
        $q = $this->db->get_where('sales', array('due_date <=' => $today, 'due_date !=' => '1970-01-01', 'due_date !=' => NULL, 'deleted'=>0, 'payment_status' => 'pending'));
...

    {
        ...
        $q = $this->db->get_where('sales', array('due_date <=' => $today, 'due_date !=' => '1970-01-01', 'due_date !=' => NULL, 'deleted'=>0, 'payment_status' => 'partial'));
    ...
    function sent_email($details) {
        ...
        if ($details) {
            ...
            $tables = $this->cron_model->yesterday_report('full');
    ...
    private function yesterday_report($range = NULL) {
    ...
    private function getCosting($date = NULL, $warehouse_id = NULL)

    {
        ...
        if ($warehouse_id) {
            $this->db->join('sales', 'sales.id=costing.sale_id')->where('sales.deleted',0)
