...
class Pos_model extends CI_Model
{
    ...
    public function getWHProduct($code, $warehouse_id)
    {
        $this->db->select('products.*, warehouses_products.quantity, categories.id as category_id, categories.name as category_name')
            ...
            ->group_by('products.id,warehouses_products.quantity');
                        ...
                        if (isset($item_cost['date'])) {
                           ...
                           // die();
                            ...
                            foreach ($item_cost as $ic) {
                                ...
                                //var_dump($ic);
                                //var_dump($ic['pi_overselling']);
                                
                                if(! isset($ic['pi_overselling']) && !empty($ic['pi_overselling'])) {
...
//echo 'hello';
    ...
    public function getSuspendedSaleItems_table($id)
    {
        $q = $this->db->get_where('savetable_items', array('suspend_id' => $id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getOpenBillByID($id)
    {
        ...
        $q = $this->db->query("SELECT s.*,t.table_name,t.no_of_guest,t.address,t.phone,t.email FROM ".$this->db->dbprefix('suspended_bills')." s LEFT JOIN ".$this->db->dbprefix('table')." t ON t.id = s.table_id WHERE s.id=$id");
        //var_dump($this->db);
    ...
    public function getOpenBillByID_table($id)
    {

        $q = $this->db->get_where('savetable_bills', array('id' => $id), 1);
        //var_dump($this->db);
    ...
    public function savetable_count()
    {
        if (!$this->Owner && !$this->Admin) {
            $this->db->where('created_by', $this->session->userdata('user_id'));
        }
        return $this->db->count_all_results("savetable_bills");
    }
    public function fetch_bills($limit, $start)
    {
        $sql = "SELECT s.*,t.table_name,t.no_of_guest,t.address,t.phone,t.email FROM ".$this->db->dbprefix('suspended_bills')." s LEFT JOIN ".$this->db->dbprefix('table')." t ON t.id = s.table_id";
        if (!$this->Owner && !$this->Admin) {
            $sql .= " WHERE created_by=$this->session->userdata('user_id')";
        }
        //$this->db->limit($limit, $start);
    ...
    public function fetch_tables($limit, $start)
    {
        if (!$this->Owner && !$this->Admin) {
            $this->db->where('created_by', $this->session->userdata('user_id'));
        }
        ...
        $query = $this->db->get("savetable_bills");
    ...
    public function suspendSale($data = array(), $items = array(), $did = NULL)
    {
     ...
     public function tableSale($data = array(), $items = array(), $did = NULL)
    {
        $sData = array(
            'count' => $data['total_items'],
            'biller_id' => $data['biller_id'],
            'table_id' => $data['table_id'],
            'no_guest' => $data['no_of_guest'],
            'customer_id' => $data['customer_id'],
            'warehouse_id' => $data['warehouse_id'],
            'customer' => $data['customer'],
            'date' => $data['date'],
        ...
        if ($did) {
            ...
            if ($this->db->update('savetable_bills', $sData, array('id' => $did)) && $this->db->delete('savetable_items', array('suspend_id' => $did))) {
                $addOn = array('suspend_id' => $did);
                end($addOn);
                foreach ($items as &$var) {
                    $var = array_merge($addOn, $var);
                }
                if ($this->db->insert_batch('savetable_items', $items)) {
                    return TRUE;
                }
            }
            ...
            if ($this->db->insert('savetable_bills', $sData)) {
                $suspend_id = $this->db->insert_id();
                $addOn = array('suspend_id' => $suspend_id);
                end($addOn);
                foreach ($items as &$var) {
