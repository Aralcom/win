    ...
    function index()
    {
       ...
       $this->form_validation->set_rules('site_logo', lang("site_logo"), 'xss_clean');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('site_name' => DEMO ? 'Business Management System' : $this->input->post('site_name'),
                ...
                'default_email' => DEMO ? 'noreply@sma.rozhsoft.com' : $this->input->post('email'),
                'language' => $lang,
                ...
                'backup_time' => trim($this->input->post('backup_time')),
                'backup_email' => trim($this->input->post('backup_email')),
                ...
                'apis' => $this->input->post('apis') ? $this->input->post('apis') : 0,
                'shop_feature' => $this->input->post('shop_feature') ? $this->input->post('shop_feature') : 0,
                'report_delete_button' => $this->input->post('report_delete_button') ? $this->input->post('report_delete_button') : 0,
...
'text_total_bal' => $this->input->post('text_total_bal'),
'text_grand_total' => $this->input->post('text_grand_total'),
'due_payment_notification' => $this->input->post('due_payment_notification'),
'customer_group_opt' => $this->input->post('customer_group_opt'),
            );
...
/*added by sudip end*/
    ...
    function updates()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $updates = get_remote_contents($protocol.'api.rozhsoft.com/v1/update/', $fields);
...
function capital()
    {
        ...
        $query = $this->db->query("SELECT *  FROM  `sma_capital` ");
        $this->data['oCapital']=$query->result() ;
        
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => 'capital'));
        $meta = array('page_title' => 'Capital', 'bc' => $bc);
        $this->page_construct('settings/capital', $meta, $this->data);
    }
    ...
    function add_capital()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->session->set_flashdata('error', validation_errors());
            redirect("system_settings/capital");
        }

        if ($this->form_validation->run() == true && $this->settings_model->addCapital($data)) { //check to see if we are creating the customer
            $this->session->set_flashdata('message', "Capital Added");
            redirect("system_settings/capital");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['page_title'] = lang("new_currency");
            $this->load->view($this->theme . 'settings/add_capital', $this->data);
        }
    }
    
    function edit_capital($id = NULL)
    {
        $this->form_validation->set_rules('date', 'date', 'trim|required');
        $this->form_validation->set_rules('description', 'description', 'trim|required');
        $this->form_validation->set_rules('value', 'value', 'required|numeric');
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateCapital($id, $data)) { //check to see if we are updateing the customer
            $this->session->set_flashdata('message', "Capital Updated");
            redirect("system_settings/capital");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            //$this->data['currency'] = $this->settings_model->getCurrencyByID($id);
            $query = $this->db->query("SELECT *  FROM  `sma_capital` where id=$id ");
            $this->data['oCapital']=$query->result() ;
            
            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_capital', $this->data);
        }
    }
    
    function delete_capital($id = NULL)
    {
        ...
        if ($this->settings_model->deleteCapital($id)) {
         // echo $this->lang->line("Capital Entry Deleted");
$this->sma->send_json(array('error' => 0, 'msg' => "Capital Entry Deleted"));
//$this->session->set_flashdata('message', "Capital Entry Deleted");
             redirect("system_settings/capital");
        }else{
            ...
            if (!empty($_POST['val'])) {
                ...
                if ( $this->input->post('form_action') == 'export_pdf') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $sc = $query->result() ;
//                       echo $sc[0]->date .' '. $sc[0]->description .' '.$sc[0]->value.' '.$sc[0]->type;
//                       die();
                        //$sc = $this->settings_model->getCurrencyByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sc[0]->date);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $sc[0]->description);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $sc[0]->value);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $sc[0]->type);
                        $row++;
                    }

                    $this->excel->getActiveSheet()->getColumnDimension('B')->setWidth(20);
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    $filename = 'capital_' . date('Y_m_d_H_i_s');
    ...
    function login_report(){
        ...
        $user = $this->getUsers_drop_down();
        $user_arr = json_decode($user);
        //var_dump($user_arr->aaData);
        $user_id = $this->input->get('user_id');
        if(isset($user_id) && !empty($user_id)){
            $login_report = $this->getloginreport_id($user_id);
           // $login_report =  $this->getTables($user_id);
        }else{
           $login_report = false; 
        }
        $this->data['user_id'] = $user_id;
        $this->data['user'] = $user_arr->aaData;
        $this->data['login_report'] = $login_report;
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('tables')));
        $meta = array('page_title' => lang('tables'), 'bc' => $bc);
        $this->page_construct('settings/login_report', $meta, $this->data);
    }
    function getloginreport_id($user_id){
       $sql = "SELECT u.username,u.email,CONCAT(MOD(HOUR(TIMEDIFF(out_time, log_time)), 24), ' Hours - ',MINUTE(TIMEDIFF(out_time, log_time)), ' Minutes - ',second(TIMEDIFF(out_time, log_time)), ' Seconds') as total_time,log.* FROM `sma_login_logout_report` as log,sma_users as u  where u.id=log.user_id and log.user_id='".$user_id."' ";
       //echo $sql;
        $query = $this->db->query($sql);
        return $query->result() ;
    }
    function getTables($user_id)
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select($this->db->dbprefix('users').".username,".$this->db->dbprefix('login_logout_report').".log_status,log_time")
            ->from("login_logout_report")
            ->join('users', 'users.id=login_logout_report.user_id')
