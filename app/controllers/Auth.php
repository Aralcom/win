...
$this->allow_reg = $this->Settings->allow_reg;
    ...
    function tables(){
        ...
        $this->page_construct('auth/tables', $meta, $this->data);
    }
...
//var_dump($m);
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
                ...
                if(!empty($user_id)){
                    $this->enter_login_report($user_id);
                }
                redirect($referrer);
            } else {
...
//var_dump($m);
    ...
    function enter_login_report($user_id){
        $query = "INSERT INTO `sma_login_logout_report` (`id`, `user_id`, session_id ,`log_status`, `log_time`,out_time) VALUES (NULL, '".$user_id."', '".session_id()."' ,'I', now(), now())";
        if($this->db->query($query)){
            $last_insert_id = $this->db->insert_id();
            $this->session->set_userdata('log_report', $last_insert_id);
            return true;
        }else{
            return false;
        }
    ...
    function out_login_report($user_id){
        $log_id = $this->session->userdata('log_report');
        $update = "update `sma_login_logout_report` set out_time=now() where id='".$log_id."'";
               
        //$query = "INSERT INTO `sma_login_logout_report` (`id`, `user_id`, session_id, `log_status`, `log_time`) VALUES (NULL, '".$user_id."','".session_id()."', 'O', now())";
    ...
    function log(){
       // var_dump(session_id());
        ...
        // var_dump($log_id);
         if($user_id){
        $query = " SELECT TIME_TO_SEC(TIMEDIFF(now(),out_time)) as time_diff  FROM `sma_login_logout_report` where  id='".$log_id."'";
        //echo $query;
            $result = $this->db->query($query)->row();
            //var_dump($result);
            //echo $result->time_diff;
            //die();
            if($result->time_diff < 900){
                $update = "update `sma_login_logout_report` set out_time=now() where id='".$log_id."'";
