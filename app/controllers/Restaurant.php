...
class Restaurant extends MY_Controller
{
    ...
    function getTables()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('table').".id as id, table_name, no_of_guest, email, phone, address, active")
            ->from("table")
            ->group_by('table.id')
            //->where('company_id', NULL)
            ->edit_column('active', '$1__$2', 'active, id')
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . site_url('restaurant/edit_table/$1') . "' class='tip' title='" . lang("edit_user") . "'><i class=\"fa fa-edit\"></i></a></div>", "id");
    ...
    function tables(){
        ...
        $this->page_construct('restaurant/tables', $meta, $this->data);
    }
    ...
    function delete_avatar($id = NULL, $avatar = NULL)
    {
        ...
        if (!$this->ion_auth->logged_in()){
            ...
            if(file_exists('assets/uploads/avatars/' . $avatar) ){
                unlink('assets/uploads/avatars/' . $avatar);
               
            }
			if(file_exists('assets/uploads/avatars/thumbs/' . $avatar)){
				 unlink('assets/uploads/avatars/thumbs/' . $avatar);
			}
			
            $this->db->update('table', array('avatar' => NULL), array('id' => $id));
    ...
    function get_table_list(){
        ...
        $row = $this->table_model->getTableRecord();
        return $row;
    }
    function getTable($id = NULL)
    {
        $row = $this->table_model->getTableByID($id);
        $this->sma->send_json(array(array('id' => $row->id, 'text' => $row->table_name)));
    }
    function tableDetail($id)
    {
        $this->sma->checkPermissions('index', true);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['table'] = $this->table_model->getTableByID($id);
        //var_dump($this->data);
    ...
    function suggestions($term = NULL, $limit = NULL)
    {
        ...
        $rows['results'] = $this->table_model->getTableSuggestions($term, $limit);
    ...
    function edit_table($id = NULL)
    {
        ...
        $this->data['title'] = lang('edit_table');

        //$table = $this->ion_auth->table($id)->row();
       $table = $this->get_table_details($id)->row();
        //var_dump($table);
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('restaurant/tables'), 'page' => lang('tables')), array('link' => '#', 'page' => lang('edit_table')));
        $meta = array('page_title' => lang('edit_table'), 'bc' => $bc);
        $this->page_construct('restaurant/edit_table', $meta, $this->data);
    }
    ...
    function login($m = NULL)
    {
        ...
        //var_dump($m);
            ...
            $this->load->view($this->theme . 'restaurant/login', $this->data);
        }
    }
    ...
    function change_password()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect('restaurant/profile/' . $user->id . '/#cpassword');
        } else {
            if (DEMO) {
    ...
    function activate($id, $code = false)
    {
        ...
        if ($id) {
            $activation = $this->table_model->activate($id);
        }

        if ($activation) {
            $this->session->set_flashdata('message', $this->table_model->messages());
                redirect($_SERVER["HTTP_REFERER"]);
        } else {
            $this->session->set_flashdata('error', $this->table_model->errors());
             redirect($_SERVER["HTTP_REFERER"]);
        }
    }
    function deactivate($id = NULL)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            if ($this->input->post('deactivate')) {
				...
				$this->data['user'] = $this->get_table_details($id)->row();
                $this->data['modal_js'] = $this->site->modal_js();
                $this->load->view($this->theme . 'restaurant/deactivate', $this->data);
            }
        } else {
            ...
            if ($this->input->post('confirm') == 'yes') {
                ...
                if ($this->ion_auth->logged_in()) {
                    $this->table_model->deactivate($id);
                    $this->session->set_flashdata('message', $this->table_model->messages());
                }
            }
    ...
    function create_table()
    {
        ...
        $this->form_validation->set_rules('no_of_guest', 'No of Guest', 'required');
        ...
        if ($this->form_validation->run() == true && $this->table_model->register($data)) {
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : ($this->table_model->errors() ? $this->table_model->errors() : $this->session->flashdata('error')));
           
            $bc = array(array('link' => site_url('home'), 'page' => lang('home')), array('link' => site_url('restaurant/tables'), 'page' => lang('tables')), array('link' => '#', 'page' => lang('create_table')));
            $meta = array('page_title' => lang('tables'), 'bc' => $bc);
            $this->page_construct('restaurant/create_table', $meta, $this->data);
        }
    }
    function update_table($id = NULL)
    {
      ...
      /*  if ($user->email != $this->input->post('email')) {
        ...
        if ($this->form_validation->run() === TRUE && $this->table_model->update($table->id, $data)) {
            $this->session->set_flashdata('message', lang('user_updated'));
            redirect("restaurant/edit_table/" . $id);
        } else {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER["HTTP_REFERER"]);
        }
    ...
    function update_avatar($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->table_model->updateAvatar($id, $photo)) {
            unlink('assets/uploads/avatars/' . $table->avatar);
            unlink('assets/uploads/avatars/thumbs/' . $table->avatar);
    ...
    function register()
    {
       ...
       $this->form_validation->set_rules('confirm_password', lang('confirm_password'), 'required');
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data)) {
            ...
            $this->load->view($this->theme .'restaurant/register', $this->data);
    ...
    function update_pos_resize(){
        
        if(!$this->session->userdata('user_id')){
            return false;
        }
        
        $insert_chk = "select count(*) as insert_chk from sma_pos_shorting where user_id='".$this->session->userdata('user_id')."'";
        $res = $this->db->query($insert_chk)->row();
        if(isset($res->insert_chk) && $res->insert_chk > 0){
            if(isset($_REQUEST) && !empty($_REQUEST)){
           $update = "update sma_pos_shorting set left_shorting='".$_REQUEST['json_shortable']."',right_shorting='".$_REQUEST['json_shortable1']."',box_size='".$_REQUEST['json_width']."',box_open_opt='".$_REQUEST['json_boxObj']."' where user_id='".$this->session->userdata('user_id')."'";
           $res = $this->db->query($update);
            ...
            if(isset($_REQUEST) && !empty($_REQUEST)){
            $insert = "INSERT INTO sma_pos_shorting (user_id,left_shorting,right_shorting,box_size,box_open_opt) VALUES ('".$this->session->userdata('user_id')."','".$_REQUEST['json_shortable']."','".$_REQUEST['json_shortable1']."','".$_REQUEST['json_width']."','".$_REQUEST['json_boxObj']."')";
    ...
    function add()
    {
        $this->sma->checkPermissions(false, true);
            ...
            $this->data['table_list'] = $this->get_table_list();
            $this->data['modal_js'] = $this->site->modal_js();
           
            $this->load->view($this->theme . 'restaurant/add', $this->data);
