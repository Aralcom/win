...
class Sales extends MY_Controller
{
    ...
    public function getdeletedSales($warehouse_id = null)
    {
...
<li>' . $sms_link . '</li>
        ...
        if ($warehouse_id) {
            $condition = array('warehouse_id'=> $warehouse_id,'deleted'=> 1);
            $this->datatables
...
<li>' . $sms_link . '</li>
        ...
        $this->load->library('datatables');
        if ($warehouse_id) {
            $condition = array('warehouse_id'=> $warehouse_id,'deleted'=> 0);
            $this->datatables
                ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, attachment, return_id")
                ->from('sales')
                //->where('warehouse_id', $warehouse_id);
    ...
    public function modal_view($id = null)
    {
        ...
        $this->data['return_sale'] = $this->sales_model->returnSales($id);
        $this->data['sale_model'] = $this->sales_model;
    ...
    public function return_sale($id = null)
    {
        ...
        $total_returned = 0;
        $return_sales = $this->sales_model->returnSales($id);
        if($return_sales){
            foreach($return_sales as $return_sale_item){
                $total_returned -= $return_sale_item->return_sale_total;
            }
        }
//var_dump($total_returned);
        if (($sale->grand_total-$total_returned) <= 0) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $amount_paid = $this->input->post('amount-paid') ? $this->input->post('amount-paid') : 0;
            $note = $this->sma->clear_tags($this->input->post('note'));
        ...
        if (($sale->grand_total - ($total_returned-$total)) >= 0 && $this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment, $si_return)) {
    ...
    public function delete($id = null)
    {
        ...
        if ($this->sales_model->deleteSale($id)) {
            if ($this->input->is_ajax_request()) {
                $this->sma->send_json(array('error' => 0, 'msg' => lang("sale_deleted"), 'ff' => site_url('pos/sales')));
                redirect('welcome');
            }else{
	 ...
	 public function delete_sales_deleted($id = null)
    {
        $this->sma->checkPermissions(null, true);
        ...
        if ($this->sales_model->deleteSaleDe($id)) {
            if ($this->input->is_ajax_request()) {
                $this->sma->send_json(array('error' => 0, 'msg' => lang("sale_deleted"), 'ff' => site_url('pos/sales')));
                redirect('welcome');
            }else{
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $pr[] = array('id' => ($c + $r), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 
                    'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'image' => $row->image);
                ...
                $datasa = array_values($_data);
             
            
           // echo"<pre>"; print_r($pr); die;
            $this->sma->send_json($datasa);
        } else {
    ...
    public function expire_gift_cards()
    {
        $this->sma->checkPermissions();
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('gift_cards')));
        $meta = array('page_title' => lang('gift_cards'), 'bc' => $bc);
        $this->page_construct('sales/expire_gift_cards', $meta, $this->data);
    }
 ...
 public function getExpireGiftCards()
    {
...
//$no = $_REQUEST['cn'];
//echo $no;
//var_dump($this->site->getGiftCardByNO($no));
    ...
    public function add_gift_card()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('card_no' => $_REQUEST['card_no'],
...
//var_dump($this->input->post('card_no'));
        if ($this->form_validation->run() == true) {
            $gift_card = $this->site->getGiftCardByID($id);
            $customer_details = $this->input->post('customer') ? $this->site->getCompanyByID($this->input->post('customer')) : null;
            $customer = $customer_details ? $customer_details->company : null;
