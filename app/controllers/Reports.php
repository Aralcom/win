...
class Reports extends MY_Controller
{
    ...
    function index()
    {
     ...
     public function pos_sales_delete_report(){
        ...
        $this->page_construct('sales/pos_sales_delete_report', $meta, $this->data);
    }
    ...
    function yearly_profit($year, $month= NULL, $warehouse_id = NULL)
    {
        if ( ! $this->Owner) {
            $this->session->set_flashdata('error', lang('access_denied'));
            $this->sma->md();
    ...
    $this->data['costing'] = $this->reports_model->getCostingYear(NULL, $warehouse_id, $year, $month);
    $this->data['discount'] = $this->reports_model->getOrderDiscountYear(NULL, $warehouse_id, $year, $month);
    $this->data['expenses'] = $this->reports_model->getExpensesYear(NULL, $warehouse_id, $year, $month);
    $this->data['returns'] = $this->reports_model->getReturnsYear(NULL, $warehouse_id, $year, $month);
    if($month) {
        $this->data['date'] = date('Y', strtotime($year . '-' . $month . '-' . '01'));
    }
    else {
        $this->data['date'] = $year;
    }

        $this->load->view($this->theme . 'reports/yearly_profit', $this->data);
    }
    ...
    function yearly_sales($warehouse_id = NULL, $year = NULL, $pdf = NULL, $user_id = NULL)
    {
        $this->sma->checkPermissions();
        if (!$this->Owner && !$this->Admin && $this->session->userdata('warehouse_id')) {
            $warehouse_id = $this->session->userdata('warehouse_id');
        ...
        for($i=1;$i<11;$i++){
            $year_loop[] = $start_year+$i;
        }
        
        $year_to_json = json_encode($year_loop);
        //var_dump($year_to_json); 
        $this->load->language('calendar');
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['year'] = $year_to_json;
         $this->data['selected_year'] = $selected_year;
        
      //var_dump($user_id);
        $get_object_array = $user_id ? $this->reports_model->getStaffYearlySales($user_id, $year_to_json, $warehouse_id) : $this->reports_model->getYearlySales($year_to_json, $warehouse_id);
        if(count($get_object_array)>0){
            foreach ($get_object_array as $key => $value) {
                $loop_year = (array)$value;
                $result[$loop_year['date']] = $loop_year;
            }
        }
$this->data['sales'] =  $result;
        if ($pdf) {
            $html = $this->load->view($this->theme . 'reports/monthly', $this->data, true);
            $name = lang("monthly_sales") . "_" . $year . ".pdf";
            $html = str_replace('<p class="introtext">' . lang("reports_calendar_text") . '</p>', '', $html);
        ...
        $bc = array(array('link' => base_url(), 
            'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('yearly_sales_report')));

        $meta = array('page_title' => lang('yearly_sales_report'), 'bc' => $bc);
        $this->page_construct('reports/yearly', $meta, $this->data);
    ...
    function restCurrenciesAll(){
        //
        $query = $this->db->query("SELECT *  FROM  `sma_currencies` ");
        $result=$query->result() ;
    ...
    $query = $this->db->query("SELECT *  FROM  `sma_currencies` WHERE code != ( SELECT default_currency FROM  `sma_settings` ) ");
    $result=$query->result() ;
    echo json_encode($result);
    ...
    function getPaymentsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ...
                ->group_start()->where('sales.deleted', 0)->or_where('sales.deleted', null)->group_end()
            ...
            if ($supplier) {
                $this->db->where('purchases.supplier_id', $supplier);
                $this->db->where('payments.purchase_id !=', null);
            }
            if ($biller) {
                $this->db->where('sales.biller_id', $biller);
            }
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT({$this->db->dbprefix('payments')}.date, '%Y-%m-%d %T') as date, " . $this->db->dbprefix('payments') . ".reference_no as payment_ref, " . $this->db->dbprefix('sales') . ".reference_no as sale_ref, " . $this->db->dbprefix('purchases') . ".reference_no as purchase_ref, paid_by, amount, type, {$this->db->dbprefix('payments')}.id as id,{$this->db->dbprefix('companies')}.name as comname")
                ->from('payments')
                ->join('sales', 'payments.sale_id=sales.id', 'left')
                ->join('purchases', 'payments.purchase_id=purchases.id', 'left')
                ->join('companies', 'sales.biller_id=companies.id', 'left')
                //->where('companies.name IS NOT NULL',NULL)
                ->group_start()->where('sales.deleted', 0)->or_where('sales.deleted', null)->group_end()
                ->group_by('payments.id');
    ...
    function profit_loss($start_date = NULL, $end_date = NULL)
    {
        ...
        $this->data['total_capital'] = $this->reports_model->getTotalCapital($start, $end);
    ...
    public function getExpensesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if($warehouse){
                $this->datatables->where($this->db->dbprefix('expenses').'.warehouse_id', $warehouse);
            }
            if ($category) {
